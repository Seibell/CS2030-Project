...
public class Simulate6 {
    ...
    public Simulate6(int numServers,
            List<Pair<Double, Supplier<Double>>> inputTimes, int maxQLength) {
        ...
        for (int i = 0; i < numServers; i++) {
            servers = servers.add(new Server(i + 1, maxQLength));
        }
    ...
    public String run() {
        ...
        while (!temp.isEmpty()) {
            ...
            if (temp.poll().first().isRealEvent() == false &&
                    temp.poll().first().execute(tempShop).first()
                            .orElseThrow().getEvent().contains("serve")) {
